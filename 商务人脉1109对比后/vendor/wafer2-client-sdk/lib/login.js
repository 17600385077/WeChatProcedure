...
function login (opts) {
  ...
  // opts.loginUrl = 'https://www.yxice.com/weapp/login'
    if (!opts.loginUrl) {
        ...
        success (loginResult) {
            ...
            wx.request({
                ...
                success (result) {
                    ...
                    // 成功地响应会话信息
                    Session.set(res);
                    res.userinfo.res_num = data.data.res_num;
                    res.userinfo.cam_num = data.data.cam_num;
                    opts.success(res.userinfo)
                },
                fail (err) {
                    console.error('登录失败，可能是网络错误或者服务器发生异常')
...
function setLoginUrl (loginUrl) {
  defaultOptions.loginUrl = 'https://renmaitong.adxing.com/weapp/login';
}
